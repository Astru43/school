...
void setup() {
  ...
  digitalWrite(2, HIGH); //Start charging the capasitor
...
void loop() {
  ...
  //If timer is stopped the capacitor is at 63% and we can calculate the capacitor's capacitance
  if(TIMSK1 == 0) {
    
    finish_time = time / 2 / 1000.0f; //The time the capacitor took to get to 63% in seconds
...
ISR(TIMER1_COMPA_vect) {
  ...
  //Check if the capacitor is charged 63%
  if (ux > 0.63f * 1023){
